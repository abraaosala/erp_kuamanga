...
@section('page-title', 'Escalas de Trabalho')
@section('page-subtitle', 'Turnos e horários de trabalho')
...
<div>
    ...
    <div class="table-container">
        <div class="px-6 py-4 border-b flex items-center justify-between" style="border-color: var(--border-color)">
            <h3 class="text-sm font-semibold" style="color: var(--text-main)">Escalas</h3>
            <div class="flex items-center gap-3">
                ...
                <a href="/rh/schedules/create" class="btn-primary px-3 py-1.5 text-xs">+ Nova</a>
                <span class="text-xs" style="color: var(--text-muted)">{{ $schedules->total() }} escala(s)</span>
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="app-table">
                ...
                <tbody>
                    @forelse($schedules->items() as $schedule)
                    <tr>
                        <td>
                            <span class="font-medium" style="color: var(--text-main)">{{ $schedule->name }}</span>
                        </td>
                        <td style="color: var(--text-muted)">{{ $schedule->check_in_time ? substr($schedule->check_in_time, 0, 5) : '—' }}</td>
                        <td style="color: var(--text-muted)">{{ $schedule->check_out_time ? substr($schedule->check_out_time, 0, 5) : '—' }}</td>
                        <td style="color: var(--text-muted)">{{ $schedule->break_minutes ? $schedule->break_minutes . ' min' : '—' }}</td>
                        ...
                        <td>
                            <a href="/rh/schedules/{{ $schedule->id }}/employees" class="text-xs font-medium transition-colors" style="color: var(--text-muted)" onmouseover="this.style.color='var(--accent)'" onmouseout="this.style.color='var(--text-muted)'">
                                {{ $schedule->employees_count ?? 0 }} funcionário(s)
                            </a>
                        </td>
                        <td>
                            ...
                            @else
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold uppercase tracking-wide bg-slate-500/10 text-slate-600 border border-slate-500/20">
                        ...
                        <td class="text-right">
                            <div class="flex items-center justify-end gap-2" x-data="{ confirm: false }">
                                <a href="/rh/schedules/{{ $schedule->id }}/employees" class="p-2 rounded-lg transition-all duration-200" style="color: var(--text-muted)" onmouseover="this.style.color='var(--accent)'; this.style.backgroundColor='var(--accent-soft)'" onmouseout="this.style.color='var(--text-muted)'; this.style.backgroundColor='transparent'" title="Funcionários">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/>
                                    </svg>
                                </a>
                                <a href="/rh/schedules/{{ $schedule->id }}/edit" class="p-2 rounded-lg transition-all duration-200" style="color: var(--text-muted)" onmouseover="this.style.color='var(--accent)'; this.style.backgroundColor='var(--accent-soft)'" onmouseout="this.style.color='var(--text-muted)'; this.style.backgroundColor='transparent'">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </a>
                                <form method="POST" action="/rh/schedules/{{ $schedule->id }}/delete" @submit.prevent="confirm ? $el.submit() : (confirm = true)">
                    ...
                    <tr>
                        <td colspan="8" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <p style="color: var(--text-muted)">Nenhuma escala encontrada</p>
                                <a href="/rh/schedules/create" class="font-medium" style="color: var(--accent)">Criar primeira escala</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        @php $schedules->appends(request()->query()) @endphp
        <div class="px-6 py-4 border-t flex items-center justify-between" style="border-color: var(--border-color)">
            <div class="flex items-center gap-4">
                ...
                <p class="text-xs" style="color: var(--text-muted)">
                    Mostrando {{ $schedules->firstItem() }}–{{ $schedules->lastItem() }} de {{ $schedules->total() }} escalas
                </p>
            </div>
            <div class="flex items-center gap-2">
                @if(!$schedules->onFirstPage())
                <a href="{{ $schedules->previousPageUrl() }}" class="btn-secondary px-3 py-1.5 text-xs">← Anterior</a>
                @endif
                @if($schedules->hasMorePages())
                <a href="{{ $schedules->nextPageUrl() }}" class="btn-secondary px-3 py-1.5 text-xs">Próximo →</a>
                @endif
            </div>
        </div>
    </div>

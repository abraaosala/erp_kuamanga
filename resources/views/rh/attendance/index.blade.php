...
@section('page-title', 'Frequência / Ponto')
@section('page-subtitle', 'Registo de ponto dos colaboradores')
...
<div>
    ...
    <div class="table-container">
        <div class="px-6 py-4 border-b flex items-center justify-between" style="border-color: var(--border-color)">
            <h3 class="text-sm font-semibold" style="color: var(--text-main)">Registos de Ponto</h3>
            <div class="flex items-center gap-3">
                ...
                <a href="/rh/attendance/create" class="btn-primary px-3 py-1.5 text-xs">+ Novo</a>
                <span class="text-xs" style="color: var(--text-muted)">{{ $records->total() }} registo(s)</span>
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="app-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <span class="font-medium" style="color: var(--text-main)">{{ $record->employee->name ?? '—' }}</span>
                        </td>
                        <td style="color: var(--text-muted)">{{ $record->date->format('d/m/Y') }}</td>
                        <td style="color: var(--text-muted)">{{ $record->check_in ? substr($record->check_in, 0, 5) : '—' }}</td>
                        <td style="color: var(--text-muted)">{{ $record->check_out ? substr($record->check_out, 0, 5) : '—' }}</td>
                        <td>
                            @switch($record->status)
                                ...
                                @break
                                @case('justificado')
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold uppercase tracking-wide bg-blue-500/10 text-blue-600 border border-blue-500/20">
                        ...
                        <td class="text-right">
                            <div class="flex items-center justify-end gap-2" x-data="{ confirm: false }">
                                <a href="/rh/attendance/{{ $record->id }}/edit" class="p-2 rounded-lg transition-all duration-200" style="color: var(--text-muted)" onmouseover="this.style.color='var(--accent)'; this.style.backgroundColor='var(--accent-soft)'" onmouseout="this.style.color='var(--text-muted)'; this.style.backgroundColor='transparent'">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </a>
                                <form method="POST" action="/rh/attendance/{{ $record->id }}/delete" @submit.prevent="confirm ? $el.submit() : (confirm = true)">
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <p style="color: var(--text-muted)">Nenhum registo de ponto encontrado</p>
                                <a href="/rh/attendance/create" class="font-medium" style="color: var(--accent)">Criar primeiro registo</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div class="px-6 py-4 border-t flex items-center justify-between" style="border-color: var(--border-color)">
            <div class="flex items-center gap-4">
                ...
                <p class="text-xs" style="color: var(--text-muted)">
                    Mostrando {{ $records->firstItem() }}–{{ $records->lastItem() }} de {{ $records->total() }} registos
                </p>
            </div>
            <div class="flex items-center gap-2">
                @if(!$records->onFirstPage())
                <a href="{{ $records->previousPageUrl() }}" class="btn-secondary px-3 py-1.5 text-xs">← Anterior</a>
                @endif
                @if($records->hasMorePages())
                <a href="{{ $records->nextPageUrl() }}" class="btn-secondary px-3 py-1.5 text-xs">Próximo →</a>
                @endif
            </div>
        </div>
    </div>

...
<div class="max-w-2xl mx-auto">
    ...
    <form method="POST" action="/rh/benefits" class="glass-card rounded-2xl p-6 space-y-6">
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4" x-data="{ departmentId: {{ ($old['department_id'] ?? '') !== '' ? (int) $old['department_id'] : 'null' }} }">
                ...
                <div>
                    ...
                    <select name="position_id" class="form-input no-select2">
                        ...
                        <option value="{{ $position->id }}" x-show="!departmentId || {{ $position->department_id ?: 'null' }} === departmentId" {{ ($old['position_id'] ?? '') != '' && (int)($old['position_id'] ?? -1) === (int) $position->id ? 'selected' : '' }}>{{ $position->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>

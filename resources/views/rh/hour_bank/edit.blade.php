...
@section('page-title', 'Editar Lançamento de Horas')
@section('page-subtitle', 'Alterar lançamento no banco de horas')
...
<div class="max-w-2xl mx-auto">
    ...
    <form method="POST" action="/rh/hour-bank/{{ $entry->id }}/update" class="glass-card rounded-2xl p-6 space-y-6">
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    ...
                    <select name="employee_id" required class="form-input">
                        ...
                        @foreach($employees as $emp)
                            <option value="{{ $emp->id }}" {{ $entry->employee_id == $emp->id ? 'selected' : '' }}>{{ $emp->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-semibold mb-1.5" style="color: var(--text-muted)">Data <span class="text-red-400">*</span></label>
                    <input type="date" name="date" required class="form-input" value="{{ $entry->date->format('Y-m-d') }}">
                </div>
                <div>
                    <label class="block text-xs font-semibold mb-1.5" style="color: var(--text-muted)">Horas <span class="text-red-400">*</span></label>
                    <input type="number" name="hours" step="0.25" required class="form-input" value="{{ $entry->hours }}">
                </div>
                <div class="md:col-span-2">
                    ...
                    <select name="type" required class="form-input">
                        <option value="">— Selecione —</option>
                        <option value="horas_extra" {{ $entry->type == 'horas_extra' ? 'selected' : '' }}>Horas Extras</option>
                        <option value="compensacao" {{ $entry->type == 'compensacao' ? 'selected' : '' }}>Compensação</option>
                        <option value="ajuste" {{ $entry->type == 'ajuste' ? 'selected' : '' }}>Ajuste</option>
                        <option value="saldo_inicial" {{ $entry->type == 'saldo_inicial' ? 'selected' : '' }}>Saldo Inicial</option>
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="observations" class="form-input" rows="3" placeholder="Justificativa ou observações">{{ $entry->observations }}</textarea>
                </div>
            </div>
        </div>

...
@section('page-subtitle', 'Saldo de horas dos colaboradores')
...
<div>
    ...
    <div class="mb-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="glass-card rounded-2xl p-4 flex items-center justify-between">
            <div>
                ...
                <p class="text-sm font-bold mt-1 {{ $row['balance'] > 0 ? 'text-emerald-500' : ($row['balance'] < 0 ? 'text-red-500' : 'text-slate-400') }}">
                    {{ number_format($row['balance'], 2) }} h
                </p>
            </div>
            @if($row['balance'] > 0)
    ...
    <div class="table-container">
        <div class="px-6 py-4 border-b flex items-center justify-between" style="border-color: var(--border-color)">
            <h3 class="text-sm font-semibold" style="color: var(--text-main)">Lançamentos</h3>
            <div class="flex items-center gap-3">
                ...
                <a href="/rh/hour-bank/create" class="btn-primary px-3 py-1.5 text-xs">+ Novo</a>
                <span class="text-xs" style="color: var(--text-muted)">{{ $entries->total() }} lançamento(s)</span>
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="app-table">
                ...
                <tbody>
                    @forelse($entries->items() as $entry)
                    <tr>
                        <td>
                            <span class="font-medium" style="color: var(--text-main)">{{ $entry->employee->name ?? '—' }}</span>
                        </td>
                        <td style="color: var(--text-muted)">{{ $entry->date->format('d/m/Y') }}</td>
                        <td>
                            @switch($entry->type)
                                @case('horas_extra')
                                <span class="px-2.5 py-1 rounded-full text-[11px] font-bold uppercase tracking-wide bg-emerald-500/10 text-emerald-600 border border-emerald-500/20">Horas Extras</span>
                                @break
                                @case('compensacao')
                                <span class="px-2.5 py-1 rounded-full text-[11px] font-bold uppercase tracking-wide bg-amber-500/10 text-amber-600 border border-amber-500/20">Compensação</span>
                                @break
                                @case('ajuste')
                                <span class="px-2.5 py-1 rounded-full text-[11px] font-bold uppercase tracking-wide bg-blue-500/10 text-blue-600 border border-blue-500/20">Ajuste</span>
                                @break
                                @case('saldo_inicial')
                                <span class="px-2.5 py-1 rounded-full text-[11px] font-bold uppercase tracking-wide bg-slate-500/10 text-slate-600 border border-slate-500/20">Saldo Inicial</span>
                                @break
                            @endswitch
                        </td>
                        <td>
                            <span class="font-bold {{ $entry->hours > 0 ? 'text-emerald-500' : ($entry->hours < 0 ? 'text-red-500' : 'text-slate-400') }}">
                                {{ $entry->hours > 0 ? '+' : '' }}{{ number_format((float)$entry->hours, 2) }} h
                            </span>
                        </td>
                        <td style="color: var(--text-muted)">{{ $entry->observations ?: '—' }}</td>
                        <td class="text-right">
                            <div class="flex items-center justify-end gap-2" x-data="{ confirm: false }">
                                <a href="/rh/hour-bank/{{ $entry->id }}/edit" class="p-2 rounded-lg transition-all duration-200" style="color: var(--text-muted)" onmouseover="this.style.color='var(--accent)'; this.style.backgroundColor='var(--accent-soft)'" onmouseout="this.style.color='var(--text-muted)'; this.style.backgroundColor='transparent'">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </a>
                                <form method="POST" action="/rh/hour-bank/{{ $entry->id }}/delete" @submit.prevent="confirm ? $el.submit() : (confirm = true)">
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <p style="color: var(--text-muted)">Nenhum lançamento encontrado</p>
                                <a href="/rh/hour-bank/create" class="font-medium" style="color: var(--accent)">Criar primeiro lançamento</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        @php $entries->appends(request()->query()) @endphp
        <div class="px-6 py-4 border-t flex items-center justify-between" style="border-color: var(--border-color)">
            <div class="flex items-center gap-4">
                ...
                <p class="text-xs" style="color: var(--text-muted)">
                    Mostrando {{ $entries->firstItem() }}–{{ $entries->lastItem() }} de {{ $entries->total() }} lançamentos
                </p>
            </div>
            <div class="flex items-center gap-2">
                @if(!$entries->onFirstPage())
                <a href="{{ $entries->previousPageUrl() }}" class="btn-secondary px-3 py-1.5 text-xs">← Anterior</a>
                @endif
                @if($entries->hasMorePages())
                <a href="{{ $entries->nextPageUrl() }}" class="btn-secondary px-3 py-1.5 text-xs">Próximo →</a>
                @endif
            </div>
        </div>
    </div>

...
@section('page-subtitle', 'Gerir contratos de trabalho')
...
<div>
    ...
    <div class="table-container">
        <div class="px-6 py-4 border-b flex items-center justify-between" style="border-color: var(--border-color)">
            <h3 class="text-sm font-semibold" style="color: var(--text-main)">Lista de Contratos</h3>
            <div class="flex items-center gap-3">
                ...
                <a href="/rh/contracts/create" class="btn-primary px-3 py-1.5 text-xs">+ Novo</a>
                <span class="text-xs" style="color: var(--text-muted)">{{ $contracts->total() }} contrato(s)</span>
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="app-table">
                ...
                <tbody>
                    @forelse($contracts->items() as $contract)
                    <tr>
                        <td>
                            <span class="font-medium" style="color: var(--text-main)">{{ $contract->employee->name ?? '—' }}</span>
                        </td>
                        <td style="color: var(--text-muted)">{{ $contract->employee->position->name ?? '—' }}</td>
                        <td>
                            <span class="text-xs font-semibold uppercase tracking-wide" style="color: var(--text-muted)">
                                @switch($contract->tipo_contrato)
                                    @case('indeterminado') Sem Termo @break
                                    @case('determinado') Prazo @break
                                    @case('estagio') Estágio @break
                                    @case('prestacao_servicos') Prestação Serviços @break
                                    @default {{ $contract->tipo_contrato }}
                                @endswitch
                            </span>
                        </td>
                        <td style="color: var(--text-muted)">{{ $contract->data_inicio->format('d/m/Y') }}</td>
                        <td style="color: var(--text-muted)">{{ $contract->data_fim ? $contract->data_fim->format('d/m/Y') : '—' }}</td>
                        <td style="color: var(--text-muted)">
                            @if($contract->salario_base)
                                {{ number_format($contract->salario_base, 2, ',', '.') }} AOA
                            @else
                                —
                            @endif
                        </td>
                        <td>
                            ...
                            @elseif($contract->status === 'encerrado')
                            ...
                            @else
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold uppercase tracking-wide bg-red-500/10 text-red-500 border border-red-500/20">
                        ...
                        <td class="text-right">
                            <div class="flex items-center justify-end gap-2" x-data="{ confirm: false }">
                                <a href="/rh/contracts/{{ $contract->id }}/edit" class="p-2 rounded-lg transition-all duration-200" style="color: var(--text-muted)" onmouseover="this.style.color='var(--accent)'; this.style.backgroundColor='var(--accent-soft)'" onmouseout="this.style.color='var(--text-muted)'; this.style.backgroundColor='transparent'">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </a>
                                <form method="POST" action="/rh/contracts/{{ $contract->id }}/delete" @submit.prevent="confirm ? $el.submit() : (confirm = true)">
                    ...
                    <tr>
                        <td colspan="8" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <a href="/rh/contracts/create" class="font-medium" style="color: var(--accent)">Criar primeiro contrato</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        @php $contracts->appends(request()->query()) @endphp
        <div class="px-6 py-4 border-t flex items-center justify-between" style="border-color: var(--border-color)">
            <div class="flex items-center gap-4">
                ...
                <p class="text-xs" style="color: var(--text-muted)">
                    Mostrando {{ $contracts->firstItem() }}–{{ $contracts->lastItem() }} de {{ $contracts->total() }} registrados
                </p>
            </div>
            <div class="flex items-center gap-2">
                @if(!$contracts->onFirstPage())
                <a href="{{ $contracts->previousPageUrl() }}" class="btn-secondary px-3 py-1.5 text-xs">← Anterior</a>
                @endif
                @if($contracts->hasMorePages())
                <a href="{{ $contracts->nextPageUrl() }}" class="btn-secondary px-3 py-1.5 text-xs">Próximo →</a>
                @endif
            </div>
        </div>
    </div>
